...
struct route_setting defaults[] = {
    ...
    { .ctl_name = "Pcm Playback Route",	                   .intval = 5 },
    { .ctl_name = NULL, },
};
                              ...
                              int enable)
{
	...
	LOGD("Setting %s",route[i].ctl_name);
        ctl = mixer_get_ctl_by_name(mixer, route[i].ctl_name);
        if (!ctl)
            return -EINVAL;

        if (route[i].strval) {
		LOGD("    to %s",route[i].strval);
            if (enable)
                mixer_ctl_set_enum_by_string(ctl, route[i].strval);
            else
                mixer_ctl_set_enum_by_string(ctl, "Off");
        } else {
		LOGD("    to %d",route[i].intval);
...
static int start_output_stream(struct tf101_stream_out *out)
{
    ...
    LOGD("start_output_stream: card:%d, port:%d, rate:%d, format:%d",card,port,out->config.rate,out->config.format);
...
{
//LOGD("Returning out get %d",AUDIO_CHANNEL_OUT_STEREO);
                          ...
                          float right)
{
    ...
    mixer_ctl_set_value(adev->mixer_ctls.headphone_volume, 0,
        PERC_TO_HEADSET_VOLUME(left));
    mixer_ctl_set_value(adev->mixer_ctls.headphone_volume, 1,
...
#define get_ctl(v,n) do { \
    adev->mixer_ctls.v = mixer_get_ctl_by_name(adev->mixer, n); \
    if (!adev->mixer_ctls.v) { \
        LOGE("Unable to find '%s' mixer control", #n); \
        goto error_out; \
    } \
} while (0)
    ...
    get_ctl(headphone_switch, MIXER_HEADPHONE_SWITCH);
